----------------------------------------------------------------------------------
-- MiSTer2MEGA65 Framework -- Wildbits K2 RevB0C adaptation
--
-- K2-local board abstraction.  The framework's core/QNICE/video contracts
-- remain unchanged; only the physical keyboard and external-memory backends
-- differ from the MEGA65 implementation.
--
...
port (
   ...
   -- K2 keyboard scanner: one low-active bit per M2M logical key number.
   ...
   -- K2 DDR3 MIG application interface.  mem_clk_i is MIG's ui_clk.
   ...
   -- K2-local OSM hotkey selection: the core (or the board shell) picks which key(s)
   -- drive the framework's menu-open bit (qnice_keys bit 7).  Consumed by k2_m2m_keyb
   -- only; no M2M modification is required.  Defaults reproduce classic Help-only.
...
end entity framework_k2;

architecture synthesis of framework_k2 is
...
-- 16-bit, word-addressed arbiter output
...
signal mem_writedata           : std_logic_vector(127 downto 0);
signal mem_byteenable          : std_logic_vector(15 downto 0);
signal mem_burstcount          : std_logic_vector(7 downto 0);
signal mem_readdata            : std_logic_vector(127 downto 0);
signal mem_readdatavalid       : std_logic;
signal mem_waitrequest         : std_logic;
signal mem_readcredit          : std_logic;
...
begin
   ...
   -- K2-local equivalent of m2m_keyb.  The board scanner supplies the whole
   -- logical-key bitmap directly, so no MEGA65 smart-keyboard serial link or
   -- keyboard-LED transport is instantiated.
   i_m2m_keyb : entity work.k2_m2m_keyb
      port map (
   ...
   i_mem_width : entity work.k2_avm_increase
      ...
      port map (
         ...
         m_avm_readcredit_o    => mem_readcredit
      );

   i_mem_bridge : entity work.avm_mig_bridge
      generic map (
      ...
      port map (
         ...
         s_avm_address_i       => mem_address,
         s_avm_writedata_i     => mem_writedata,
         s_avm_byteenable_i    => mem_byteenable,
         s_avm_burstcount_i    => mem_burstcount,
         s_avm_readdata_o      => mem_readdata,
         s_avm_readdatavalid_o => mem_readdatavalid,
         s_avm_waitrequest_o   => mem_waitrequest,
         s_avm_readcredit_i    => mem_readcredit,
         app_addr_o            => mem_app_addr_o,
         app_cmd_o             => mem_app_cmd_o,
         app_en_o              => mem_app_en_o,
         app_rdy_i             => mem_app_rdy_i,
         app_wdf_data_o        => mem_app_wdf_data_o,
         app_wdf_end_o         => mem_app_wdf_end_o,
         app_wdf_mask_o        => mem_app_wdf_mask_o,
         app_wdf_wren_o        => mem_app_wdf_wren_o,
         app_wdf_rdy_i         => mem_app_wdf_rdy_i,
         app_rd_data_i         => mem_app_rd_data_i,
         app_rd_data_valid_i   => mem_app_rd_data_valid_i,
         app_rd_data_end_i     => mem_app_rd_data_end_i
      );
